...
DWORD WINAPI  CReceiveEmailDlg::_AfxMainTestAna(LPVOID lpParam)
{
	...
	CMailAnalysis ana;
	CSQLServer sql;
	sql.Connect(pDlg->m_sqldbinfo.szDBAdd, pDlg->m_sqldbinfo.szDBName, pDlg->m_sqldbinfo.szUserName, pDlg->m_sqldbinfo.szPasswd);
...
void CReceiveEmailDlg::OnBnClickedButtonSet()
{
	...
	for (int i = 0; i < sizeof(m_hProcess) / sizeof(m_hProcess[0]);i++)
	{
		if (m_hProcess[i]==NULL)
			m_hProcess[i] = CreateThread(NULL, 0, _AfxMainTestAna, (LPVOID)this, 0, &dwId[i]);
	}
}
