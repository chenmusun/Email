...
inline LPTSTR GetErrorMessage(DWORD error, LPTSTR lpMessage)
{
	...
	case GETADDRINFO_ERROR:
		...
		break;
	case SOCKETCONNECT_ERROR:
		lstrcpy(lpMessage, _T("Connect套接字失败！"));
		break;
	default:
		break;
	}
...
inline void GetErrorMessage(int error, char* pMessage,int nLen=MAX_PATH)
{
	...
	case HOST_UNREACHABLE:
		...
		break;
	case 10038:
		sprintf_s(pMessage, nLen, "10038-在一个非套接字上尝试了一个操作");
		break;
	case 10051:
		sprintf_s(pMessage, nLen, "10051-向一个无法连接的网络尝试了一个套接字操作");
		break;
	case 10052:
		sprintf_s(pMessage, nLen, "10052-当该操作在进行中，由于保持活动的操作检测到一个 故障，该连接中断");
		break;
	case 10053:
		sprintf_s(pMessage, nLen, "10053-您的主机中的软件放弃了一个已建立的连接");
		break;
	case 10054:
		sprintf_s(pMessage, nLen, "10054-远程主机强迫关闭了一个现有的连接");
		break;
	case 10060:
		sprintf_s(pMessage,nLen,"10060-由于连接方在一段时间后没有正确的答复或连接的主机没有反应，连接尝试失败");
		break;
	case SOCKETCONNECT_ERROR:
		sprintf_s(pMessage, nLen, "Connect套接字失败！");
		break;
	default:
		break;
	}
