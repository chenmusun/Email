...
#ifdef _DEBUG
		CString csDebug;
		csDebug.Format(_T("GetGUIDErr[%s]\r\n[%s]"), (TCHAR*)e.ErrorMessage(), (TCHAR*)e.Description());
		OutputDebugString(csDebug);
#endif
	}
	...
	try
	{
		...
		case 0://混合验证模式
		{
			csCommand.Format(_T("Provider=sqloledb;Data Source=%s;Database=%s;User ID=%s;Password=%s")
				, lpServer, lpDatabase, lpUser, lpPass);
		}
			break;
		case 1://Windows验证模式
		{
...
long CSQLServer::SaveAttachment(ATTACH_FILE& attach, long lEmailID)
{
	...
	if (pos > 0)
		attach.csRemoteName.Format(_T("%s%s"), csGuid, attach.csFileName.Mid(pos));
	try
	{
